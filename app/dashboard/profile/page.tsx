  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="overflow-hidden p-0">
        <div className="h-28 w-full bg-gradient-to-r from-primary/25 via-primary/10 to-violet/20" />
        <div className="flex flex-col gap-4 px-6 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-end gap-4">
            <div className="-mt-10 flex size-20 items-center justify-center rounded-2xl border-4 border-card bg-primary font-serif text-2xl font-medium text-primary-foreground">
              {loading ? <Loader2 className="size-5 animate-spin" /> : initials}
            </div>
            <div className="pb-1">
              <h2 className="font-serif text-2xl font-medium text-foreground">{profile?.name ?? 'Loading…'}</h2>
              <p className="text-sm text-muted-foreground">
                Homeowner · Member since {profile ? formatDate(profile.createdAt) : '—'}
              </p>
            </div>
          </div>
          <Button onClick={() => setEditOpen(true)} className="bg-primary text-primary-foreground hover:bg-primary/90">
            Edit Profile
          </Button>
        </div>
      </Card>
      ...
      <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
        <Card className="space-y-6">
          <h3 className="font-serif text-lg font-medium text-foreground">Personal Information</h3>
          ...
          <div className="border-t border-border pt-6">
            <h3 className="mb-4 font-serif text-lg font-medium text-foreground">Portfolio Value</h3>
            <div className="flex items-baseline gap-3">
              <span className="font-serif text-4xl font-medium text-foreground">{formatCurrency(stats?.totalValue)}</span>
        ...
        <Card className="space-y-4">
          <h3 className="font-serif text-lg font-medium text-foreground">Account Overview</h3>
