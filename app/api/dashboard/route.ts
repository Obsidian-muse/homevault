...
export async function GET() {
  return withErrorHandling(async () => {
    ...
    const withStatus: { id: string; expiryDate: Date; assetId: string; asset: { name: string }; status: string }[] =
      allWarranties.map((w: { expiryDate: Date }) => ({ ...w, status: computeWarrantyStatus(w.expiryDate) }))
